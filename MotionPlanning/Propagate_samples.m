function [Traj] = Propagate_samples(n,D_sph,Imp_Ellipse,Asteroid)

a = Imp_Ellipse.sem(1,1); %axes of minimum ellipsoid
b = Imp_Ellipse.sem(2,1);
c = Imp_Ellipse.sem(3,1);
w = Asteroid.w_AI(3);
mu = Asteroid.mu;
...
T = [0 2000];
Y0(:,i)=D_sph(:,i);     %Initial domain space
...
q_A = [0; 0; sin(w*T/2); cos(w*T/2)];  %quaternion of inertial to asteroid frame

YY =  quat_trans(q_A,[Y(1); Y(2); Y(3); 0],'vect');    %Position of SC wrt Asteroid frame
imp = (YY(1)/a)^2 + (YY(2)/b)^2 + (YY(3)/c)^2;                    %Instantaneous ellipsoid wrt Asteroid frame
