...
<<<<<<< HEAD:ConvexOptimisation/SuccessiveConvexification/Tests/acik_plots.m
        %state convergence
        if ii<i
        diff2{ii}(1:19,k) = abs(ITR.x_k{end}(2:20,k) - ITR.x_k{ii}(2:20,k));
=======
        if ii < i
        %quat normalisation
        q_norm(ii,k) = ITR.x_k{ii}(8:11,1)'*ITR.x_k{ii+1}(8:11,1);
>>>>>>> cc67d781cf9f9418328a3cc40d3f4deeca1a23d6:ConvexOptimisation/SuccessiveConvexification/Tests/test1/acik_plots.m
        end
    end
    m_spent(ii) = ITR.x_k{ii}(1,1)-ITR.x_k{ii}(1,end);
...
for ii = 1:i-1
    for jj=1:19
    plot3(ii,t(jj),diff2{ii}(1:19,jj),'Marker','o')
    end
end
xlabel('Iterations')
ylabel('Difference in State')
zlabel('Time')
set(gca, 'YScale', 'log')
axis([1 i-1 10e-10 10e0 0 t(end)])
grid on
