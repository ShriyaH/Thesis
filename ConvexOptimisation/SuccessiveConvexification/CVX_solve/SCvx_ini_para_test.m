function [A,b,G,h,C,dims] = SCvx_ini_para_test()
...
% Discretisation
    ...
    jdx = ns+nc+(jj-1)*(n)+(1:nv);
    A(idx,jdx) = Ed;
    
    % slack virtual control discrete matrices
    jdx = ns+nc+nv+(jj-1)*(n)+(1:nsl);
    A(idx,jdx) = Fd;

    % construct b
    b(idx,1) = zd;
    if idx == (K-1)*ns+(1:ns)
        b(idx,1) = zd-xf; %final state
    end
    end   
end
...
if Switch.exact_penalty_on %||.||_1 in cost function
    if Switch.virtual_control_on % nu_k <= s_k
        ...
        for ii = 1:K        
            Gt(ii,(ii-1)*n+(24:29)) = [1 1 1 -1 -1 -1];
        end
...
if Switch.tilt_ang_on
    ...
    for ii = 1:K
        Ac = S;
        Ac(1,(ii-1)*n+(8:11)) = [1 0 0 0];
        Ac(2,(ii-1)*n+(8:11)) = [0 1 0 0];
          
        cc = s;
        cc(1,(ii-1)*n+(8:11)) = [0 0 0 0];
...
if Switch.virtual_control_on
    ...
    for ii = 1:K
        ...
        cc = s;
        cc(1,(ii-1)*n+ns+nc+nv+(1:nsl)) = -ones(1,nsl);
