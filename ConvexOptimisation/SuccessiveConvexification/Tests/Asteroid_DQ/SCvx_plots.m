...
hold on
for j = 1:i
	h1(j) = plot(rr(4*j-3,:),rr(4*j-1,:),'Marker','.');
...
hold on
for j = 1:i
comet3(rr(4*j-3,:),rr(4*j-2,:),rr(4*j-1,:));
end
xlabel('X (m)');
ylabel('Y (m)');
zlabel('Z (m)');
...
hold on
for j = 1:i
plot3(rr(4*j-3,:),rr(4*j-2,:),rr(4*j-1,:));
end
xlabel('X (m)');
ylabel('Y (m)');
zlabel('Z (m)');
