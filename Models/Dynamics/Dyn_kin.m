function [t,y,I_total_n,m_n,ch,T_SRP_T,T_GG_T,Wf] = Dyn_kin (T,Sun,Asteroid,Switch,SC,Samples,n_v,n_c)
...
%% Integrator 
 for k = 1:n_v          %for number of velocity samples
     ...
     for j = 1:n_c      %for number of state samples
         j
         T = [0 1000];
         Y0 = [[x0(1:3,j,k); 0]; [v0(1:3,j,k);0]; w_BI0; q_BI0; q_AI0];    %initial state vector
...
%% Function for state differential
 ...
 function dY = orb_int(T,Y)
    ...
    r_A = quat_trans(Y(17:20),Y(1:4),'vect')';
    [~,g_I,~,Wf] = Poly_g(r_A,Y(13:16),Y(17:20),rho,V,F,E,F_tilde,E_tilde);
    [a_3BP_I] = F_3BP(Y(1:4),r_d,mu_s,BP);
    [F_SRP_B,a_SRP_I] = F_SRP(Y(13:16),f,p,n,A,a_r,a_d,AU,vel_light,m_n,SRP);

    Y2 = [g_I + a_3BP_I + a_SRP_I; 0];
    ...
    T_SRP(1:3,i) = cross(r(1:3,i),F_SRP_B(i,:)');
        if grav_model == 1
            vert_pm = vv + Y(1:3,1)';
            [T_GG_T] = T_GG(vv,vert_pm,Y(13:16),Y(17:20),m_n,rho,V,F,E,F_tilde,E_tilde);
        else
            T_GG_T = 0;
        end
    end
